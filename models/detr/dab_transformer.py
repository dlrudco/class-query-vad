...
from utils.misc import inverse_sigmoid
from ops.modules import MSDeformAttn3D
...
def gen_sineembed_for_position(pos_tensor):
    # num_actors, bs, _ = pos_tensor.size()
    # sineembed_tensor = torch.zeros(num_actors, bs, 256)
    scale = 2 * math.pi
...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward(self, tgt, memory,
        ...
        if not self.rm_self_attn_decoder:
            ...
            # shape: num_actors x batch_size x 256
        ...
        # Apply projections here
        # shape: num_actors x batch_size x 256
        lvl_w = self.lvl_w_embed(tgt) # N_a, BT, num_levels
        lvl_w = lvl_w.softmax(-1)
        q_memory = torch.einsum("ntl,lhtc->nhtc", lvl_w, memory) # (N_a, BT, L), (L, HW, BT, C),  ->  N_a, HW, BT, C
...
class TransformerClassDecoderLayer(nn.Module):
    ...
    def forward(self, actor_feature, q_memory, pos, query_sine_embed, class_queries, orig_res, num_queries, is_first):
        ...
        h, w = orig_res
        actor_feature_expanded = actor_feature.flatten(0,1)[..., None, None].expand(-1, -1, h, w)
