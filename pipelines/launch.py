...
def spawn_workers(main, cfg):
    ...
    if cfg.DDP_CONFIG.AUTO_RANK_MATCH:
        assert len(cfg.DDP_CONFIG.WORLD_URLS) > 0
        assert cfg.DDP_CONFIG.WORLD_URLS[0] in cfg.DDP_CONFIG.DIST_URL
        assert len(cfg.DDP_CONFIG.WORLD_URLS) == cfg.DDP_CONFIG.WORLD_SIZE
        # import socket 
        # s = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
        # s.connect(("8.8.8.8", 80))
        ...
        # s.close()
        cfg.DDP_CONFIG.WORLD_RANK = get_local_ip_and_match(cfg.DDP_CONFIG.WORLD_URLS)
