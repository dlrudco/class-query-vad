...
def load_detr_weights(model, pretrain_dir, cfg):
    ...
    for k, v in checkpoint['model'].items():
        if k.split('.')[1] == 'transformer':
            ...
            # pretrained_dict.update({k: v[:query_size]})
        elif cfg.CONFIG.MODEL.SINGLE_FRAME and "refpoint_embed" in k:
    ...
    if cfg.DDP_CONFIG.GPU_WORLD_RANK == 0:
        ...
        print_log(log_path, "not found layers:", len([k for k in not_found_dict.keys() 
...
def load_model_and_states(model, scaler, cfg):
    """
    Load pretrained model weights.
    """
    if os.path.isfile(cfg.CONFIG.MODEL.PRETRAINED_PATH):
        ...
        model_dict.update(pretrained_dict)
        model.load_state_dict(model_dict, strict=False)
        print_log(log_path,"=> loaded checkpoint '{}'".format(cfg.CONFIG.MODEL.PRETRAINED_PATH))
    else:
        print_log(log_path,"=> no checkpoint found at '{}'".format(cfg.CONFIG.MODEL.PRETRAINED_PATH))
